...
export default function LiveSentimentDashboard() {
  ...
  if (!latestEvent) {
    return (
      <div className="w-full h-full flex flex-col items-center justify-center text-zinc-500 font-mono text-[11px] bg-black/20 rounded-2xl relative overflow-hidden">
        ...
        <p>Awaiting CCaaS Telemetry Stream via Transient WebSockets...</p>
      </div>
    );
  }
  ...
  return (
    <div className="w-full h-full flex gap-4 text-zinc-100 font-sans h-full pt-10">
      
      {/* Principal Card - Dark-Native Glassmorphic */}
      <div className="flex-1 flex flex-col h-full rounded-2xl">
        <div className={`flex flex-col h-full bg-zinc-950/60 backdrop-blur-xl border ${isAlarming ? 'border-red-500/40 shadow-[0_0_30px_rgba(239,68,68,0.15)] ring-1 ring-red-500/20' : 'border-white/10'} rounded-2xl overflow-hidden relative transition-all duration-500`}>
          ...
          <div className="p-6 flex flex-col flex-1 justify-between gap-6">
            ...
            <div className="space-y-5">
              ...
              <div className="relative group mt-2">
                <p className="text-[10px] text-zinc-500 uppercase tracking-widest mb-3 font-semibold flex items-center gap-2">
                  <MessageSquareQuote className="w-3 h-3"/> Active Transcript
                </p>
                <div className="p-5 rounded-xl bg-black/40 border border-white/5 relative shadow-inner">
            ...
            {/* Sentiment Footer & Automated Workflow Triggers */}
            <div className="rounded-xl border border-white/5 bg-zinc-900/30 p-4 shadow-xl">
              <div className="flex justify-between items-center mb-4">
                <p className="text-[10px] text-zinc-500 uppercase tracking-widest font-semibold">AI Sentiment Assessment</p>
                <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-md border ${SentimentColor(payload.sentiment)}`}>
              ...
              {/* Churn Alert / Retention Automated Workflows */}
              {isAlarming && (
                <div className="pt-4 border-t border-red-900/30 animate-in fade-in slide-in-from-bottom-2 duration-500">
                  <p className="text-[10px] text-red-500/90 uppercase tracking-widest mb-3 font-bold flex items-center gap-2">
                    <AlertTriangle className="w-3 h-3" /> Churn Risk Detected — Suggested AI Workflows
                  </p>
                  <div className="flex flex-wrap gap-2">
                    <button className="px-3 py-1.5 rounded-md text-[10px] uppercase font-bold tracking-wider bg-zinc-950 border border-red-900/50 text-red-100 hover:bg-red-900/40 transition-colors shadow-sm">
                      10% Discount (3mo)
                    </button>
                    <button className="px-3 py-1.5 rounded-md text-[10px] uppercase font-bold tracking-wider bg-zinc-950 border border-red-900/50 text-red-100 hover:bg-red-900/40 transition-colors shadow-sm">
                      Free Upgrade
      ...
      {/* Right Sidebar - Historical Stream Log */}
      <div className="w-[280px] hidden xl:flex flex-col border border-white/5 rounded-2xl bg-zinc-950/40 backdrop-blur-xl overflow-hidden relative shadow-2xl">
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-2 scrollbar-thin scrollbar-thumb-zinc-800">
          {historyEvents.length === 0 ? (
            ...
            historyEvents.map((evt, idx) => (
              <div key={evt.id || idx} className="p-3 rounded-xl bg-zinc-900/40 border border-white/5 hover:bg-zinc-800/40 transition-colors duration-300 cursor-default">
                <div className="flex justify-between items-center mb-2">
                  <span className="text-[9px] font-mono text-zinc-500 bg-black/50 px-1.5 py-0.5 rounded">{evt.agent_id}</span>
                  <SentimentIcon sentiment={evt.payload.sentiment} />
                </div>
                <p className="text-[10px] text-zinc-300 line-clamp-2 leading-relaxed italic border-l border-zinc-700 pl-2">
